...
class _ChatScreenState extends State<ChatScreen> with WidgetsBindingObserver {
  ...
  String tempFilename =
      DateTime.now().millisecondsSinceEpoch.toString(); //"TempRecording"
...
//  final file =  await ImagePicker.pickVideo(source: ImageSource.gallery);
  ...
  void _updateDeliveredStatus() async {
    ...
    await Firestore.instance
        ...
        .updateData({'status': 'delivered'}).whenComplete(() {
...
// if(result.documents.length == 0){
      ...
      responseData['share_link'] =
          'www.xpert.tv/cr/${widget.xpertData['slug']}';

      DocumentReference newIntentDoc = Firestore.instance
          .collection('xpert_master')
...
// building messages MAP
  _buildMessage(var message, var response, var rsptype, int index) {
    ...
    Container botMessage = message == ""
        ? Container(height: 0.0)
        : onScreenQuestionsShow[index]
            ? Container(
            ...
            ? Container(
                ...
                decoration: BoxDecoration(
                  image: DecorationImage(
                      image: videothumbnailCounter < _thumbnailPaths.length
                          ? FileImage(
                              File(_thumbnailPaths[videothumbnailCounter]))
                          : AssetImage('assets/def_prof_pic.png'),
                      fit: BoxFit.cover),
                  borderRadius: BorderRadius.all(Radius.circular(15.0)),
                ),
                child: Center(
                  child: !onScreenLoading[index]
                      ? Container(
                          height: 20.0,
                          width: 20.0,
            ...
            : rsptype == "audio"
                ? Container(
                    margin: EdgeInsets.only(
                        top: MediaQuery.of(context).size.height * 0.015, //8.0
                        bottom:
                            MediaQuery.of(context).size.height * 0.015, //8.0
                        left: MediaQuery.of(context).size.width * 0.30, // 80.0
                        right: MediaQuery.of(context).size.width * 0.012),
                    // padding: EdgeInsets.symmetric(horizontal: 25.0, vertical: 15.0),
                    width: MediaQuery.of(context).size.width * 0.75,
                            ...
                            : IconButton(
                                ...
                                onPressed: () {
                                  Navigator.push(
                                      context,
                                      MaterialPageRoute(
                                          builder: (context) => ChatAudioPlayer(
                                                audioUrl: response.toString(),
                                              )));
                                },
                              ),
                      ],
    ...
    return Container(
      child: Column(
        ...
        children: <Widget>[botMessage, responseMsg],
      ),
    );
  }
  ...
  _buildMessageComposer() {
    for (int i = 0; i < liveResponses.length; i++) {
      ...
      print('Live responses with index: ' +
          liveResponses[i].toString() +
          i.toString());
    }
                  ...
                  await Navigator.push(
                      ...
                      MaterialPageRoute(
                          builder: (context) => CameraExampleHome(
                                incomingQuestion: messages[messageCounter],
                                docId: null,
                                orderDocId: null,
                              ))).then((url) {
                    if (url != null && url.toString().isNotEmpty) {
                      setState(() {
                        ...
                        isSending = true;
                        liveResponses
                            .add(url); // actually this is the local path
                        responseTypes.add('video');
                        // messageCounter++;
                      });
                      ...
                      child: Row(
                        ...
                        children: <Widget>[
                          ...
                          IconButton(
                            ...
                            onPressed: () {
                              print(
                                  'Msg counter: ' + messageCounter.toString());
                              if (liveResponses.length < messages.length &&
                                  !isSending) {
                                setState(() {
                                  isSending = true;
  ...
  void _fetchBotIntentMessages() async {
    await Firestore.instance
      ...
      setState(() {
        ...
        if (messages.isNotEmpty) onScreenQuestionsShow[0] = true;
  ...
  startRecording() async {
    try {
      ...
      if (await AudioRecorder.hasPermissions) {
        ...
        Fluttertoast.showToast(
            msg: "Error! Audio recorder lacks permissions.",
            backgroundColor: Colors.grey,
            textColor: Colors.white,
      ...
      setState(() {
        ...
        Fluttertoast.showToast(
            msg:
                'Recording Your Audio message! Tap the Mic button again to stop.',
            backgroundColor: Colors.grey,
            textColor: Colors.white,
            toastLength: Toast.LENGTH_SHORT);
            ...
            appBar: AppBar(
              title: Row(
                children: <Widget>[
                  CircleAvatar(
                    radius: 25.0,
                    backgroundImage: widget.botPicUrl != null
                        ? NetworkImage(widget.botPicUrl.toString())
                        : AssetImage('assets/def_prof_pic.png'),
                  ),
                  SizedBox(
                    width: 8.0,
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (BuildContext context, int index) {
                              ...
                              if (index < liveResponses.length) {
                                print('RSP type LEN: ' +
                                    responseTypes.length.toString());
                                response = liveResponses[index];
                                rspType = responseTypes[index];
                              }
                              return _buildMessage(
                                  message, response, rspType, index);
