...
class _NotificationPageState extends State<NotificationPage> {
            ...
            child: RichText(
              text: TextSpan(
                  ...
                  children: <TextSpan>[
                    ...
                    TextSpan(
                        text: data['reaction_type'] == 'like'
                            ? ' liked your answer to'
                            : data['reaction_type'] == 'view'
                                ? ' viewed your answer for'
                                : data['reaction_type'] == 'save'
                                    ? ' saved your answer for'
                                    : data['reaction_type'] == 'follow'
                                        ? ' has started following you'
                                        : data['reaction_type'] == 'comment'
                                            ? ' shared a comment "${data['user_comment']}" for your answer to'
                                            : data['reaction_type'] == 'share'
                                                ? ' has shared your answer for'
                                                : data['reaction_type'] ==
                                                        'messaged'
                                                    ? ' chatted with your bot and said "${data['utterance']}"'
                                                    : data['reaction_type'] ==
                                                            'chat'
                                                        ? ' has started a chat with your bot'
                                                        : ''),
                    TextSpan(
                        text: data['reaction_type'] == 'messaged'
                            ? ''
                            : data['reaction_type'] == 'chat'
                                ? ''
                                : ' "' +
                                    unescape
                                        .convert(data['question'].toString()) +
                                    '"',
                        style: TextStyle(fontWeight: FontWeight.bold))
                  ]),
  ...
  void _updateNotificationField() async {
    await Firestore.instance
        ...
        .updateData({'notification': 'seen'}).whenComplete(() {
